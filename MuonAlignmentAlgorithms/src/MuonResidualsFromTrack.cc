...
MuonResidualsFromTrack::MuonResidualsFromTrack(const edm::EventSetup& iSetup,
    ...
    double maxResidual,
    bool fillLayerPlotDT, bool fillLayerPlotCSC, struct DTLayerData * layerData_DT, TTree * layerTree_DT, struct CSCLayerData * layerData_CSC, TTree * layerTree_CSC)
: m_recoTrack(recoTrack)
{
  ...
  for ( std::vector<TrajectoryMeasurement>::const_iterator iTrajMeasurement =  vTrajMeasurement.begin(); iTrajMeasurement != vTrajMeasurement.end(); ++iTrajMeasurement ) {
    nTrajMeasurement++;
	...
	if ( trajMeasurementHitId.det() == DetId::Tracker ) {
	  ...
	  if (    trajMeasurementHitId.subdetId() == StripSubdetector::TID || trajMeasurementHitId.subdetId() == StripSubdetector::TEC) {
...
/*
	...
	//This was belonging to the old residual computation, when you was refitting the muon track using muon information. Not used anymore.
	//If you are in Muon System
	  ...
	  if ( trajMeasurementHitId.subdetId() == MuonSubdetId::DT ) {
		...
		for ( std::vector<const TrackingRecHit*>::const_iterator itDTSeg2D =  vDTSeg2D.begin(); itDTSeg2D != vDTSeg2D.end(); ++itDTSeg2D ) {
		  std::vector<const TrackingRecHit*> vDTHits1D =  (*itDTSeg2D)->recHits();
		  if (m_debug) std::cout << "            vDTHits1D size: " << vDTHits1D.size() << std::endl;
		  for ( std::vector<const TrackingRecHit*>::const_iterator itDTHits1D =  vDTHits1D.begin(); itDTHits1D != vDTHits1D.end(); ++itDTHits1D ) {
		...
		if ( vCSCHits2D.size() >= 5 ) {
		  for ( std::vector<const TrackingRecHit*>::const_iterator itCSCHits2D =  vCSCHits2D.begin(); itCSCHits2D != vCSCHits2D.end(); ++itCSCHits2D ) {
	...
	if ( hitId2.det() == DetId::Tracker ) {
	  ...
	  //NOW CSC
		...
		if ( vCSCHits2D.size() >= 5 ) {
		  ...
		  for ( std::vector<TrackingRecHit*>::const_iterator itCSCHits2D =  vCSCHits2D.begin(); itCSCHits2D != vCSCHits2D.end(); ++itCSCHits2D ) {
