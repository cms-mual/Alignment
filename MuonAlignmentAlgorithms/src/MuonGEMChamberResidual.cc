...
MuonGEMChamberResidual::MuonGEMChamberResidual(edm::ESHandle<GlobalTrackingGeometry> globalGeometry,
...
void MuonGEMChamberResidual::addResidual(edm::ESHandle<Propagator> prop,
  ...
  if (m_debug)
    std::cout << "MuonGEMChamberResidual::addResidual 1" << std::endl;
  DetId id = hit->geographicalId();
  if (m_debug)
    std::cout << "MuonGEMChamberResidual::addResidual 2" << std::endl;
  const GEMGeometry *GEMGeometry = dynamic_cast<const GEMGeometry *>(m_globalGeometry->slaveGeometry(id));
  if (m_debug)
    std::cout << "MuonGEMChamberResidual::addResidual 3" << std::endl;
  assert(GEMGeometry);

  if (m_debug) {
    std::cout << " MuonGEMChamberResidual hit->localPosition() x: " << hit->localPosition().x()
  ...
  if (m_debug)
    std::cout << " MuonGEMChamberResidual residual: " << residual << std::endl;
