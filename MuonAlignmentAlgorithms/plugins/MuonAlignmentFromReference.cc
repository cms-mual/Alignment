        ...
        for (ConstTrajTrackPairCollection::const_iterator trajtrack = trajtracks.begin();  trajtrack != trajtracks.end();  ++trajtrack)
        {
            ...
            if (m_minTrackPt < track->pt()  &&  track->pt() < m_maxTrackPt && m_minTrackP < track->p()  &&  track->p() < m_maxTrackP)
            {
                ...
                if ( fabs(track->dxy(eventInfo.beamSpot().position())) < m_maxDxy )
                {
                    ...
                    MuonResidualsFromTrack muonResidualsFromTrack(iSetup, magneticField, globalGeometry, muonDetIdAssociator_, prop, traj, track, m_alignableNavigator, 1000., m_createLayerNtuple_DT, m_createLayerNtuple_CSC, &layerData_DT, m_ttree_DT_layers, &layerData_CSC, m_ttree_CSC_layers);
			...
			if (m_allowTIDTEC  ||  !mrft.contains_TIDTEC())
			{
				...
				if ((int)chamberIds.size() >= m_minNCrossedChambers)
				{
					...
					for (std::vector<DetId>::const_iterator chamberId = chamberIds.begin();  chamberId != chamberIds.end();  ++chamberId)
					{
						...
						// DT 4th station
						else if (m_doDT && chamberId->subdetId() == MuonSubdetId::DT && DTChamberId(chamberId->rawId()).station() == 4){
							MuonChamberResidual *dt13 = mrft.chamberResidual(*chamberId, MuonChamberResidual::kDT13);
							...
							if (dt13 != nullptr)
							{
								...
								if (dt13->numHits() >= m_minDT13Hits)
								{
									...
									if (fitter != m_fitters.end())
									{
										...
										residdata[MuonResiduals5DOFFitter::kWeightOccupancy] = myOccupancyMap.GiveCorrection(DTChamberId(chamberId->rawId()).wheel(), DTChamberId(chamberId->rawId()).station(), DTChamberId(chamberId->rawId()).sector(), dt13->trackx(),dt13->tracky());
										if (m_debug) 
										{
...
void MuonAlignmentFromReference::fitAndAlign(){
    ...
    for (const auto& ali: m_alignables)
    {
        ...
        { // ME 1/1 and ME 1/4 are the same chambers but are divided. here you are fitting them as a single chamber.
...
void MuonAlignmentFromReference::writeTmpFiles()
{
    ...
    for (std::vector<unsigned int>::const_iterator index = m_indexes.begin();  index != m_indexes.end();  ++index, ++i){
        ...
        fitter->write(file, i);  //All the other informations are written inside this method (you just need the file and the index to the element to write)
    }
